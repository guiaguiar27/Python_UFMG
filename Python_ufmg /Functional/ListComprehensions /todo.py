...
def delInitials(L):
    ...
    """
    # TODO 
    return [c.capitalize()  for c in L if len(c) > 1] 
...
def everyOccurrence(S, Q):
    ...
    """
    #TODO  
    return [i for i in range(len(S)) for j in range(len(Q)) if S[i] == Q[j]]
def factors(N):
    ...
    """
    # TODO
    return [c  for c in range(2,N) if N%c == 0 and c != N ] 
...
def isPerfect(N):
    ...
    if functools.reduce(operator.add,factors(N),1) != N : return False  
    else : return True  
